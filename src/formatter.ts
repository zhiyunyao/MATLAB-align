...
const REG_WS = /\s/;
const REG_CONTROL = /^\s*(for|function)\b/;
...
export default class Formatter {
  ...
  public process(editor: vscode.TextEditor): void {
    ...
    editor.selections.forEach((sel) => {
      ...
      const globalMode = this.getConfig().get("globalMode");
    ...
    editor.edit((editBuilder) => {
      for (let i = 0; i < ranges.length; ++i) {
        ...
        let infos = ranges[i].infos;
        let lastline = infos[infos.length - 1].line;
        let location = new vscode.Range(infos[0].line.lineNumber,
          0,
          lastline.lineNumber,
    ...
    while (pos < text.length) {
      ...
      if (currTokenType == TokenType.Block) {
        ...
        while (pos < text.length) {
          ...
          if (bracket == char) {
            ++bracketCount;
          } else if (bracket == BRACKET_PAIR[char]) {
            if (bracketCount == 1) {
              break;
            } else {
              --bracketCount;
  ...
  protected hasSameIndent(info1: LineInfo, info2: LineInfo): boolean {
    ...
    let t1 = info1.tokens[0];
    let t2 = info2.tokens[0];
  ...
  protected format(range: LineRange): string[] {
    ...
    // 1. Remove indentatioin, and trailing whitespace
    ...
    // 2. Remove whitespace surrounding operator.
    for (let info of range.infos) {
      let i = 1;
    ...
    while (exceed < rangeSize) {
      ...
      for (let l = 0; l < rangeSize; ++l) {
        ...
        for (; i < end; ++i) {
          ...
          // Vertical align will occur at significant operator
    ...
    if (configWS["comment"] < 0) {
      ...
      for (let l = 0; l < rangeSize; ++l) {
        ...
        if (info.tokens.length) {
          let res = result[l];
          let token: any = info.tokens.pop();
          result[l] = res + whitespace(resultSize - res.length + configWS["comment"]) + token.text;
        }
      }
    }
